package com.example.weatherap.api
...
class WeatherService(private val context: Context) {
    ...
    suspend fun getLocation(name: String): LatLng? =
        withContext(Dispatchers.IO) {
            val location = search(name) ?: return@withContext null
            val latitude = location.lat ?: return@withContext null
            val longitude = location.lon ?: return@withContext null
    ...
    private fun search(query: String): APILocation? {
        ...
        return if (!apiLoc.isNullOrEmpty()) {
            apiLoc[0]
        } else {
            null
        }
    ...
    suspend fun getWeather(
        name: String
    ): APICurrentWeather? = withContext(Dispatchers.IO) {
    ...
    suspend fun getForecast(
        name: String
    ): APIWeatherForecast? = withContext(Dispatchers.IO) {
